...
hubs<-read_excel("./Data/Analysis_Food Hubs.xlsx", sheet = "Data")

hubs<- hubs %>%
  select(Hub, Type)

farms<-farms %>%
  right_join(hubs, by="Hub") %>%
  filter(!is.na(Lat))
...
ggsave("allhubs.png", device = "png", path="./Analysis/Output/", dpi = 600)

np.hub<-ggplot()+
  geom_sf(data=County, fill="white", color="gray")+
  geom_sf(size=1.25, fill=NA, data=State)+
  geom_sf(size=1.5, data=subset(farms.map, farms.map$Type=="Nonprofit")) +
  scale_colour_manual(values=cbbPalette)+
  geom_point(aes(Hub_Lon, Hub_Lat, color = Hub), size=4.0, subset(farms, farms$Type=="Nonprofit"))+
  geom_segment(aes(x = Lon, y = Lat,
                   xend = Hub_Lon, yend = Hub_Lat, color=Hub),
               data=subset(farms, farms$Type=="Nonprofit"))+
  xlab("")+
  ylab("")+
  ggtitle("Farm to Hub Connections: Nonprofit Hubs")+
  theme_bw()+
  theme(axis.text.x = element_blank(),
        axis.text.y = element_blank(),
        axis.ticks = element_blank(), 
...
ggsave("nphubs.png", device = "png", path="./Analysis/Output/", dpi = 600)

cp.hub<-ggplot()+
  geom_sf(data=County, fill="white", color="gray")+
  geom_sf(size=1.25, fill=NA, data=State)+
  geom_sf(size=1.5, data=subset(farms.map, farms.map$Type=="Coop")) +
  scale_colour_manual(values=cbbPalette)+
  geom_point(aes(Hub_Lon, Hub_Lat, color = Hub), size=4.0, subset(farms, farms$Type=="Coop"))+
  geom_segment(aes(x = Lon, y = Lat,
                   xend = Hub_Lon, yend = Hub_Lat, color=Hub),
               data=subset(farms, farms$Type=="Coop"))+
  xlab("")+
  ylab("")+
  ggtitle("Farm to Hub Connections: Cooperatives")+
  theme_bw()+
  theme(axis.text.x = element_blank(),
        axis.text.y = element_blank(),
        axis.ticks = element_blank(), 
...
ggsave("cphubs.png", device = "png", path="./Analysis/Output/", dpi = 600)

fp.hub<-ggplot()+
  geom_sf(data=County, fill="white", color="gray")+
  geom_sf(size=1.25, fill=NA, data=State)+
  geom_sf(size=1.5, data=subset(farms.map, farms.map$Type=="For Profit")) +
  scale_colour_manual(values=cbbPalette)+
  geom_point(aes(Hub_Lon, Hub_Lat, color = Hub), size=4.0, subset(farms, farms$Type=="For Profit"))+
  geom_segment(aes(x = Lon, y = Lat,
                   xend = Hub_Lon, yend = Hub_Lat, color=Hub),
               data=subset(farms, farms$Type=="For Profit"))+
  xlab("")+
  ylab("")+
  ggtitle("Farm to Hub Connections: For Profit Hubs")+
  theme_bw()+
  theme(axis.text.x = element_blank(),
        axis.text.y = element_blank(),
        axis.ticks = element_blank(), 
...
ggsave("fphubs.png", device = "png", path="./Analysis/Output/", dpi = 600)
